...
void internal_node_split(Table* table, uint32_t need_split_page_num) {

    void* old_node = get_page(table->pager, need_split_page_num);// old
    uint32_t parent_page_num = *node_parent(old_node); // 确定现在internal的父母
    ...
    *node_parent(new_node) = *node_parent(old_node); //复制父亲
    ...
    *leaf_node_next_leaf(old_node) = new_page_num; // old节点的sibling为新节点
    ...
    // 1 <= internal_node_num <= 2
    uint32_t old_node_num_keys = *internal_node_num_keys(old_node);
    uint32_t med_key = INTERNAL_NODE_MAX_CELLS / 2; // 1

    /*
     * 这里偷懒，使用保留 old 和 new，然后直接设置cell_num保证可以写入
     * new中进行前移
     * */
    for (int32_t i = old_node_num_keys; i > med_key; i--) {
        void* destination_node = new_node;

        uint32_t index_within_node = i % (INTERNAL_NODE_MAX_CELLS - 1); // i % 2
        void* destination = leaf_node_cell(destination_node, index_within_node);
        memcpy(destination, leaf_node_cell(new_node, i), LEAF_NODE_CELL_SIZE);
    }
    *(internal_node_num_keys(old_node)) = med_key;
    *(internal_node_num_keys(new_node)) = med_key;

    // 其实根本不用思考提升谁，因为提升的这个东西是一个虚假的数据，只是一个数字，真正的索引还是放在叶子节点里
    uint32_t original_num_keys = *internal_node_num_keys(parent);
    uint32_t child_max_key = get_node_max_key(new_node);
    ...
    if (child_max_key > get_node_max_key(right_child)) {
        ...
        *internal_node_child(parent, index) = new_page_num;
